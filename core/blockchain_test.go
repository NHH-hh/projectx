...
func TestAddBlock(t *testing.T) {
	...
	for i := 0; i < lenBlock; i++ {
		b := randomBlockWithSignature(t, uint32(i+1), getPrevBlocHash(t, bc, uint32(i+1)))
		assert.Nil(t, bc.AddBlock(b))
	}
	assert.Equal(t, uint32(lenBlock), bc.Height())
	assert.Equal(t, lenBlock+1, len(bc.headers))
	assert.NotNil(t, bc.AddBlock(randomBlock(uint32(89), types.RandomHash())))
}

func TestAddBlockToHeight(t *testing.T) {
	bc := newBlockchainWithGenesis(t)
	assert.Nil(t, bc.AddBlock(randomBlockWithSignature(t, uint32(1), getPrevBlocHash(t, bc, uint32(1)))))
	assert.NotNil(t, bc.AddBlock(randomBlockWithSignature(t, uint32(3), types.RandomHash())))
}
